...
- [x] Home management — create/list (`/homes`), view/edit/delete (`/homes/[homeId]`), scoped by `ownerId` (`src/server/homes.ts`).
- [x] Room/location management — create/edit/delete nested under a home (`src/server/rooms.ts`), ownership checked via the parent home on every mutation.
- [ ] Asset CRUD
